...
public class GuiTransferCommandHandler extends CommandHandler<SQLException> {
    ...
    protected boolean checkSync(CommandSender sender, String[] args) {
        ...
        if (sender instanceof Player p) {
            if (!p.hasPermission("tcgui.gui.transfer") && !p.hasPermission("tcgui.admin.gui.transfer")) {
        ...
        if (args.length < 4) {
            returnError(sender, "Missing argument(s): /tcgui gui transfer <gui name> <player>");
            return false;
        }
    ...
    protected boolean checkAsync(CommandSender sender, String[] args) throws SQLException {
        ...
        if (sender instanceof Player p) {
            if (!p.hasPermission("tcgui.admin.gui.transfer")) {
